    ...
    private fun setupActivityRecognition() {
        ...
        this.activityRecognitionReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                // TO-DO 6: Extract activity type from intent extras and pass it to updateActivityCard method.
                // Take a look at ActivityRecognitionService to see how intent extras are formed.

                var extras = intent.getIntExtra(ActivityRecognitionService.ACTIVITY_EXTRA, 0);

                updateActivityCard(extras);

            }
        }

        // TO-DO 7: Register created receiver only for ActivityRecognitionService.INTENT_ACTION.
        registerReceiver(activityRecognitionReceiver, IntentFilter(ActivityRecognitionService.INTENT_ACTION));
    }

    private fun updateMap(location: Location) {
        if (googleMap != null) {
//             TO-DO 3: Clear current marker and create a new marker based on the received location object.

            googleMap!!.clear();

            var coords = LatLng(location.latitude, location.longitude);

            var newMarker = MarkerOptions().position(coords).title("Noua locatie");
//             TO-DO 4: Use CameraUpdateFactory to perform a zoom in.

            googleMap!!.addMarker(newMarker);

            googleMap!!.animateCamera(CameraUpdateFactory.newLatLngZoom(coords, 10F));
        }
    }
